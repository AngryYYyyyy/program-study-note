...
CAS（Compare-and-Swap）是一种无锁的同步策略。它包括三个操作数：内存位置（V）、预期原值（A）和新值（B）。如果内存位置的值与预期原值相匹配，那么处理器会自动将该位置值更新为新值。

如果需要同时更新多个共享变量，单个CAS操作无法处理，需要使用复杂的算法结构，如事务内存。
...
- **initialValue()**：当线程首次调用 `get()` 但还没有副本时，用来提供初始值。默认返回 `null`，可以被重写。
...
public class ThreadLocalExample {
    ...
    // 创建一个 Integer 类型的 ThreadLocal 实例
    private static final ThreadLocal<Integer> threadLocalCount = ThreadLocal.withInitial(() -> 0);

    public static void main(String[] args) {
        Thread t1 = new Thread(() -> updateThreadLocalCount());
        Thread t2 = new Thread(() -> updateThreadLocalCount());
        t1.start();
        t2.start();
    }

    private static void updateThreadLocalCount() {
        for (int i = 0; i < 5; i++) {
            Integer count = threadLocalCount.get(); // 获取当前线程的计数器
            count++;
            threadLocalCount.set(count); // 更新计数器
        ...
        threadLocalCount.remove(); // 避免内存泄露
    }
}
```
...
- **value**：如果不适当地管理（如未调用 `remove()` 方法），那么长时间运行的线程可能会导致旧的 `ThreadLocal` 变量副本不被回收，从而引起内存泄漏。
...
#### ① Java对象头和Mark Word
Java中每个对象在堆内存中都有一个对象头。对象头包含两部分：一部分是 `Mark Word`，它存储对象自身的运行时数据（如哈希码、锁信息、分代年龄等）；另一部分是类型指针，指向其类元数据。`Mark Word` 是实现锁升级的关键，它标记了对象当前的锁状态。
...
<img src="./assets/image-20240627105902466.png" alt="image-20240627105902466" style="zoom: 80%;" />

1. **无锁状态/匿名偏向**：
   - 当没有线程同步访问某个对象时，对象处于无锁状态。
...
3. **轻量级锁**：
   - 当有另一个线程尝试获取已偏向的锁时，偏向锁需要撤销，随后进入轻量级锁状态。
   - 轻量级锁通过在当前线程的栈帧中创建一个锁记录（Lock Record）来实现。
   - 锁记录中存储了锁对象原来的 `Mark Word`，并尝试使用 CAS 操作将对象头的 `Mark Word` 更新为指向锁记录的指针。
   - 如果成功，当前线程获得锁；如果失败，表示有竞争存在。

4. **重量级锁**：
   - 当轻量级锁的CAS操作多次失败，表明锁竞争激烈，此时轻量级锁会升级为重量级锁。
   - 重量级锁通过操作系统的互斥量来实现，每个对象的 `Mark Word` 指向一个重量级锁（如互斥量）。
   - 重量级锁最耗性能，因为涉及到操作系统的线程调度和上下文切换。
...
3. **再次尝试获取锁**：当锁的持有者释放锁，它会检查Entry Set、Wait Set，这些线程将再次尝试获取锁。
...
- **灵活性**：ReentrantLock 提供了比 synchronized 更多的方法和功能，如尝试非阻塞获取锁（tryLock）、可中断的锁获取（lockInterruptibly）以及公平锁或非公平锁的支持。
-  **性能**：在竞争较少的情况下，synchronized 的性能可能略优于 ReentrantLock，因为 JVM 对 synchronized 块的优化（如锁消除、锁粗化等）。但在高竞争环境下，ReentrantLock 的性能通常更胜一筹，因为它分散了请求锁的线程，减少了阻塞。
- **功能**：ReentrantLock 提供了一些 synchronized 不具备的功能，例如等待条件（Condition），允许更细粒度的线程协调和控制。
...
- **Wait Queue**：当线程尝试获取同步状态失败时，它们会被加入到一个等待队列中。队列是一个 FIFO 队列，并且底层为双向链表，保证了第一个请求资源的线程将是第一个获取资源的线程（公平锁情况下）。

#### **② 操作方法**

- **acquire(int arg)**：尝试以独占方式获取对象状态。如果获取失败，当前线程将被加入到队列中，并可能会进入阻塞状态。
- **release(int arg)**：释放给定的状态值，如果释放后允许其他线程成功持有状态，则会唤醒队列中的一个或多个线程。
- **acquireShared(int arg)**：尝试以共享方式获取对象状态。这种方式允许多个线程同时获取状态。
- **releaseShared(int arg)**：以共享方式释放对象状态，和独占方式类似，释放状态后将唤醒等待中的线程。

#### **③ 条件对象（Condition Objects）**

AQS 还支持条件对象，这是一个类似于 Object.wait()/notify() 的机制，允许线程在某些条件下等待，直到它们被另一个线程唤醒。使用条件对象可以构建类似于 ConditionVariable 的同步结构，这对于实现某些特定的同步需求非常有用。
...
![image-20240627164048878](./assets/image-20240627164048878.png)
...
// NonfairSync是Sync的一个非公平同步实现的静态内部类
...
![image-20240627181041215](./assets/image-20240627181041215.png)
